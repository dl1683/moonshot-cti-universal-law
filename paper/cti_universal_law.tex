...
This decouples two distinct universality phenomena: (1) near-simplex geometry ($\rho\approx0.46$) is \emph{substrate-independent} across all modalities tested; (2) the cross-family shift in $\alpha$ ($\alpha_\mathrm{decoder}\approx1.5$, $\alpha_\mathrm{audio}\approx4.7$, $\alpha_\mathrm{encoder}\approx7$, $\alpha_\mathrm{ViT}\approx0.6$) is not explained by $\rho$ and requires a second renormalization factor (likely effective dimensionality or $\kappa$ normalization conventions).
...
\textbf{(2) Constant universality within family}: $\alpha$ is stable within architecture families (NLP decoders: CV$=0.019$/$0.023$; 19 architectures); across families, $\alpha$ varies ($\alpha_\mathrm{NLP}\approx1.48$, $\alpha_\mathrm{ViT}\approx0.63$, $\alpha_\mathrm{CNN}\approx4.4$, $\alpha_\mathrm{audio}\approx4.7$), with signal modality the leading predictor (continuous-signal $\alpha\approx4$--$5$ vs.\ discrete-token $\alpha\approx1.5$).
...
Per-family values: $\alpha_\mathrm{decoder}\approx1.48$, $\alpha_\mathrm{encoder}\approx7.7$, $\alpha_\mathrm{ViT}\approx0.63$, $\alpha_\mathrm{CNN}\approx4.4$, $\alpha_\mathrm{audio}\approx4.7$; the law applies to any architecture when appropriate pooling is used.
...
Universality is of the \emph{functional form}: the constant $\alpha$ is modality-specific ($\alpha_\mathrm{NLP}\approx1.48$, $\alpha_\mathrm{ViT}\approx0.63$, $\alpha_\mathrm{CNN}\approx4.4$, $\alpha_\mathrm{audio}\approx4.7$), with continuous-signal modalities (vision, audio) converging at $\alpha\approx4$--$5$ and discrete-token NLP at $\alpha\approx1.5$; cross-task drift and honest-scope limitations (including protein LM failure and human fMRI null) are enumerated in \S\ref{sec:limitations}.
