...
The Gumbel race mechanism is the same as the mechanism underlying extreme-value statistics in physics (e.g., maxima of random fields). Within the NLP decoder family, $\alpha$ is stable across architectures; whether it plays a role analogous to a universal constant or correlation length across all modalities remains to be shown given the modality-dependent values ($\alpha_\mathrm{NLP}=1.48$, $\alpha_\mathrm{ViT}=0.63$, $\alpha_\mathrm{CNN}=4.4$, $\alpha_\mathrm{audio}=4.7$). The audio-vision convergence ($\alpha_\mathrm{audio}\approx\alpha_\mathrm{CNN}$) hints at a deeper structure: continuous-signal modalities may share a universal $\alpha$, distinct from discrete-token NLP.
