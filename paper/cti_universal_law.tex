...
\paragraph{Retracted equicorrelation analysis.}
We retract the former universal-equicorrelation, near-simplex-universality, and zero-parameter $\alpha(\rho)$ claims.
The implementation used for the historical result multiplied projected centroid differences by $\sqrt{\Lambda}$, applying $\Sigma_W^{1/2}$ rather than the stated inverse-whitening transform $\Sigma_W^{-1/2}$.
Matched labeled-cloud nulls place the historical band inside central 99\% null intervals, while a shared-anchor isotropic-centroid calculation gives $\rho\to1/2$ without learned or semantic structure.
The corrected real-data comparison could not be run because the retained caches contain scalar summaries but no aligned embeddings and labels; consequently there is no survival evidence for the claim.
\footnote{For provenance only, the legacy estimator reported six cross-modal values in $[0.455,0.467]$ with mean $0.462$ (CV$=1.0\%$; \texttt{results/cti\_cross\_modal\_rho.json}). These measurements are retained as historical data, not as an invariant or an approximately universal result.}

\paragraph{Cross-family constants.}
...
A controlled synthetic validation generated equicorrelated Gaussian class-conditional data ($K\in\{4,10,20\}$, $d=50$, generator parameter $\rho\in[0,0.8]$, 12 signal strengths). The logit-linear form achieves mean $R^2=0.982$ (min $0.967$) across the generator sweep (\texttt{results/cti\_synthetic\_gumbel\_validation.json}), supporting the functional form in that controlled setting. The fitted $\alpha$ showed no systematic dependence on the generator's $\rho$; this experiment therefore provides no support for an $\alpha(\rho)$ prediction.
...
The source of this cross-modality variation remains open. The retracted equicorrelation analysis cannot explain it; hypotheses involving within-class covariance structure require independent testing.
...
\textbf{Retracted competition-geometry analysis:} The earlier equicorrelation and near-simplex interpretations of the Allen data used the same incorrect whitening transform and are retracted. The biological results above concern the association between $\kappa_\mathrm{nearest}$ and decoding performance; they do not provide evidence for a universal equicorrelation parameter.
...
The previously reported cross-modal equicorrelation constant, near-simplex universality, and $\alpha(\rho)$ explanation are retracted because the estimator applied covariance amplification rather than inverse whitening and the historical range is compatible with matched shared-anchor nulls. The retained evidence does not support an approximate-universality fallback.
