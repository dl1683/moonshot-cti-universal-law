...
Cross-modality validation finds the same functional form for ViT-Large ($R^2=0.964$), ResNet50 on CIFAR, and 7 frozen speech models on Speech Commands ($r=0.898$, $p=0.006$, $K=36$).
...
Near-simplex competition geometry ($\rho\approx0.46$, CV$=1.0\%$ across 6 modalities: NLP, audio, vision, and mouse cortex) provides a \emph{zero-parameter mean-level} prediction of $\alpha$ for NLP decoders: pooling $\rho$ across 11 architectures and 3 datasets ($K=4,14,77$; 200 bootstrap resamples each), $\hat\alpha_\mathrm{pred}=1.540$ vs.\ observed $1.477$ ($+4.3\%$ error, zero free parameters).
Per-model $\rho$ does not monotonically predict per-model $\alpha$ ($r=-0.55$, $n=11$); however, the observed $\alpha$ CV ($2.3\%$) is fully consistent with LOAO regression estimation noise (expected $\approx2.8\%$ from 176 training points per fold), so there is no residual per-model signal for $\rho$ to explain --- $\alpha$ is a true constant within NLP decoders. Encoder $\alpha$ is $4$--$5\times$ higher for the same $\rho$, indicating a second renormalization factor beyond $\rho$.
...
\textbf{Honest scope:} $\alpha$ varies by signal type (NLP decoders: 1.48; continuous-signal modalities: ViT 4.5, CNN 4.0, audio 4.7); universality is of the \emph{functional form} only.
...
\item \textbf{Generation law}: The same Gumbel-race mechanism extends to next-token prediction: $\kappa$ from the unembedding matrix $W_U$ (zero forward passes) predicts $\log\mathrm{CE}$ across 22 models spanning Transformers, SSMs, Hybrids, and novel architectures ($r=-0.55$, $p=0.008$; $r=-0.70$ excluding one novel-architecture outlier). Within a fixed-$V$ group (Pythia + Mamba, $n=10$), $r=-0.84$ with no significant architecture interaction ($F$-test $p=0.147$): Transformers and SSMs lie on the same regression line. The vocabulary-size term vanishes ($\beta_\mathrm{gen}\approx 0$) because softmax competition spontaneously reduces $V$-way to $K_\mathrm{eff}\approx 2$--$3$-way.
\end{enumerate}

\paragraph{Practical implications.}
The law enables several zero-cost or low-cost diagnostics.
\textbf{Architecture ranking}: computing $\kappa_\mathrm{nearest}$ from frozen embeddings (no downstream labels, no probing) correctly ranks 9 architectures by retrieval quality ($\rho=0.833$, $p=0.005$; pre-registered).
\textbf{Layer selection}: within a model, $\kappa_\mathrm{nearest}$ selects the optimal representation layer for retrieval without any downstream evaluation (mean $\rho=0.640$, 10/10 positive across held-out models and datasets).
\textbf{Generation diagnostics}: $\kappa$ from the unembedding matrix $W_U$ alone (zero forward passes) predicts perplexity ordering across architectures, enabling model comparison before any text generation.
\textbf{Calibrated prediction}: 4 probe measurements per new architecture reduce absolute prediction error by 86\%.
These applications follow directly from the law's structure --- they are not additional hypotheses but consequences of the geometric mechanism.

\section{Related Work}

\paragraph{Neural scaling laws and universality.}
Power-law regularities for language-model loss are well-established \citep{kaplan2020,hoffmann2022}, though these scaling laws are unreliable for downstream task performance \citep{lourie2025scaling}. Our aim is analogous but targets geometry-to-accuracy transfer within individual models.
...
Classical EVT describes minima/maxima asymptotics \citep{fisher1928,gnedenko1943,gumbel1958}. The CTI derivation applies this to nearest-neighbor distance minima among competing classes. Independent convergent evidence: \citet{zhang2025gps} use Gumbel priors for set-to-set matching in few-shot classification, confirming that nearest-neighbor distance extrema follow Gumbel distributions.

\paragraph{Neural-collapse geometry and nearest-class dominance.}
Late-training geometry and nearest-class structure are central in neural collapse \citep{papyan2020,han2022,zhu2021,garrido2023,rangamani2023,wu2024linguistic,sukenik2025nc}. Geometry-based transferability metrics (RankMe \citep{garrido2023}, spectral order parameters \citep{liu2025}) predict downstream performance via effective rank and inter/intra-class geometry; multiplicative logit adjustment \citep{ye2024} uses NC geometry for boundary correction.
...
However, per-model $\rho$ does not monotonically predict per-model $\alpha$ ($r=-0.55$, $p=0.08$, $n=11$). This is expected: the observed $\alpha$ CV ($2.28\%$) is fully consistent with LOAO regression estimation noise (expected SE$/\bar\alpha \approx 2.8\%$ from 176 training points per fold), so there is no residual per-model signal for $\rho$ to explain. The formula captures the \emph{true} $\alpha$ to $+4.3\%$; the per-model ``variation'' is estimation noise, not architecture-specific structure.
Remarkably, the same $\rho\approx0.46$ appears across \emph{all tested modalities}: mouse visual cortex ($\rho=0.466$, Allen Neuropixels $K=118$; \S\ref{subsec:comprehensive-universality}), audio speech models (WavLM-Base+ $\rho=0.455$, HuBERT-Base $\rho=0.464$; Speech Commands $K=36$), vision (ViT-Base $\rho=0.458$ on CIFAR-10; ResNet50 $\rho=0.467$ on CIFAR-100), and NLP encoders (BERT-base, ELECTRA-small: $\rho=0.446$--$0.456$).
Pooling all 6 modalities gives $\bar\rho=0.462$, CV$=1.0\%$ (range $[0.455, 0.467]$; \texttt{results/cti\_cross\_modal\_rho.json}; Figure~\ref{fig:cross-modal-rho}) --- the tightest invariant in this work.
This decouples two distinct universality phenomena: (1) near-simplex geometry ($\rho\approx0.46$) is \emph{substrate-independent} across all modalities tested; (2) the cross-family shift in $\alpha$ (NLP decoders $\approx1.5$ vs.\ continuous-signal modalities $\approx3.3$--$5.0$: ViT $4.5$, CNN $4.0$, audio $4.7$) is not explained by $\rho$ and requires a second renormalization factor.

\begin{figure}[t]
\centering
\includegraphics[width=0.85\linewidth]{../results/figures/fig_cti_cross_modal_rho.png}
\caption{Cross-modal equicorrelation universality. The $\Sigma_W$-whitened equicorrelation $\rho$ is measured independently in 6 modalities spanning NLP (11 architectures), audio (2 speech models), vision (ViT, ResNet50), and mouse visual cortex (Neuropixels). All values fall within $[0.455, 0.467]$ (CV$=1.0\%$) --- tighter than $\alpha$'s within-decoder CV of $2.3\%$. The dashed line marks the regular-simplex ETF prediction ($\rho=0.5$); the solid line is the pooled mean $\bar\rho=0.462$. This is the tightest invariant in the paper: near-simplex competition geometry is a universal structural property of categorical representations, independent of substrate, signal type, architecture, and training procedure.}
\label{fig:cross-modal-rho}
\end{figure}
...
\toprule
Model & Family & Dataset & $K$ & $\alpha$ ($A$) & $R^2$/Pearson $r$ \\
\midrule
ViT-Base-16-224     & ViT  & CIFAR-10  & 10  & $A{=}0.592$$^\ddagger$ & $R^2=0.811$, $r=0.901$ \\
ViT-Large-16-224    & ViT  & CIFAR-10  & 10  & $A{=}0.630$$^\ddagger$ & $R^2=0.964$, $r=0.982$ \\
ViT-Base-16-224$^\dagger$  & ViT  & CIFAR-100 & 100 & 3.899 & $r=0.773$ \\
ResNet50$^\dagger$           & CNN  & CIFAR-100 & 100 & 4.418 & $r=0.749$ \\
Audio (7 models)    & 4 arch families & Speech Cmds & 36 & 4.669 & $r=0.898$ \\
\midrule
...
ViT-Large at $K=10$ reaches $R^2=0.964$, matching the NLP fit in explained variance. The ViT CIFAR-10 entries report $A$ (not $\alpha$): they fit $\logit(q)=A\cdot\kappa\cdot\sqrt{d_\mathrm{eff}}+C$, factoring out the effective dimensionality explicitly.\footnote{$^\ddagger$The CIFAR-10 ViT entries report $A$ from $\logit(q)=A\cdot\kappa\cdot\sqrt{d_\mathrm{eff}}+C$, not $\alpha$ from $\logit(q)=\alpha\cdot\kappa+C$; $A{=}0.63$ and $\alpha{=}1.477$ are not directly commensurable. When ViT is fitted with the standard parameterization ($\logit(q)=\alpha\cdot\kappa+C$), the combined ViT-Base/Large cross-layer $\alpha{=}4.49$ (\texttt{results/cti\_alpha\_family\_law.json}), consistent with CNN ($4.0$--$4.4$) and audio ($4.67$). This confirms a two-regime structure: discrete-token NLP ($\alpha{\approx}1.5$) vs continuous-signal modalities ($\alpha{\approx}3.3$--$5.0$).}

\paragraph{Audio modality.}
Seven frozen speech models from 4 architectures (Wav2Vec2, HuBERT, WavLM, Whisper; 39M--316M parameters) on Speech Commands v0.02 ($K=36$ spoken keywords, 4890 test samples; features at $\frac{2}{3}$-depth layer, PCA to 256 dimensions) yield $r=0.898$ ($p=0.006$), $\alpha_\mathrm{audio}=4.669$ ($R^2=0.807$; \texttt{results/cti\_audio\_speech.json}).
The slope $\alpha_\mathrm{audio}\approx4.7$ matches $\alpha_\mathrm{CNN}\approx4.4$ but not $\alpha_\mathrm{NLP}\approx1.5$, suggesting a two-regime structure: continuous-signal modalities (vision, audio) share $\alpha\approx4$--$5$, while discrete-token modalities (NLP) operate at $\alpha\approx1.5$. This is consistent with spectrotemporal features (audio spectrograms) sharing geometric properties with spatial features (images), both involving continuous 2D signal structure.
...
\caption{Multi-modality evidence summary. (A) LOAO $\hat\alpha$ for 12 NLP architectures (single-$C_0$ fit); CV$=0.019$, all within 6\% of the mean (pre-registered threshold 0.25). (B) $\hat\alpha$ by architecture family on log-scale: NLP decoders cluster tightly around $\approx1.48$; ViT ($\approx4.5$), CNN ($\approx4.0$), and audio ($\approx4.7$) converge; NLP encoders ($\approx7.1$) are highest. Continuous-signal modalities converge at $\alpha\approx3.3$--$5.0$; the functional form is universal, the constant is signal-type-specific. (C) Pearson $r$ vs.\ $K$: ViT-Large reaches $r\approx0.98$ at $K=10$ ($R^2=0.964$); both ViT-Base ($r=0.773$) and ResNet50 CNN ($r=0.749$) fall below the MC noise floor at $K=100$ ($E[r]=0.875$), confirming the attenuation is architecture-independent.}
...
Hypothesis scorecard: 7/13 PASS, 2 FAIL (Pythia LOAO driven by 160M outlier; whitened-$\kappa$ Proxy~B fails cross-architecture due to incomparable $\Sigma_W$),
2 partial, 1 mixed, 1 not yet tested
...
novel architectures.
\textbf{Left:} All models ($n{=}22$, $r{=}{-}0.55$, $p{=}0.008$); regression line fit on $n{=}21$ excluding LFM2.5
($r = -0.697$, $p < 0.001$).
...
(6)~\textbf{Generation law}: the same $\kappa$ geometry in the unembedding matrix $W_U$ predicts $\log(\mathrm{CE})$ across 22 models and 4 architecture types ($r=-0.55$, $p=0.008$; $r=-0.70$ excluding one outlier; Section~\ref{sec:generation-law}), extending the Gumbel-race mechanism from classification to next-token prediction.
(7)~\textbf{Audio-visual alpha convergence}: Speech models from 4 architectures ($\alpha_\mathrm{audio}=4.67$, $r=0.898$, $p=0.006$) match vision CNNs ($\alpha_\mathrm{CNN}=4.4$), revealing a two-regime structure: continuous-signal modalities share $\alpha\approx4$--$5$, while discrete-token NLP operates at $\alpha\approx1.5$.
...
\textbf{(2) Constant universality within signal type}: $\alpha$ is stable within architecture families (NLP decoders: CV$=0.019$/$0.023$; 19 architectures); across families, $\alpha$ varies by signal type: discrete-token NLP decoders give $\alpha\approx1.48$, while all continuous-signal modalities converge at $\alpha\approx3.3$--$5.0$ (ViT: $4.5$, CNN: $4.0$, audio: $4.7$).
...
Per-family values (standard $\alpha=$ slope of $\logit(q)$ vs $\kappa$): $\alpha_\mathrm{decoder}\approx1.48$, $\alpha_\mathrm{ViT}\approx4.5$, $\alpha_\mathrm{CNN}\approx4.0$, $\alpha_\mathrm{audio}\approx4.7$, $\alpha_\mathrm{encoder}\approx7.7$; the law applies to any architecture when appropriate pooling is used.
...
\paragraph{Connection to physics and synthetic validation.}
The Gumbel race mechanism is the same as the mechanism underlying extreme-value statistics in physics (e.g., maxima of random fields).
A controlled synthetic validation confirms the derivation: generating equicorrelated Gaussian class-conditional data ($K\in\{4,10,20\}$, $d=50$, $\rho\in[0,0.8]$, 12 signal strengths), the logit-linear form achieves mean $R^2=0.982$ (min $0.967$) across all $\rho$ values (\texttt{results/cti\_synthetic\_gumbel\_validation.json}). This confirms the functional form is a genuine structural prediction of the Gumbel race, not a curve-fitting artifact. Interestingly, in the isotropic Gaussian setting $\alpha$ does not depend on $\rho$: the $\alpha(\rho)=\sqrt{4/\pi}/\sqrt{1-\rho}$ relationship requires anisotropic within-class noise (structured $\Sigma_W$), which is absent in idealized data but present in real neural networks. This clarifies that $\alpha$ universality in trained networks is a non-trivial empirical regularity arising from the structured geometry of learned representations, not a trivial consequence of Gaussian statistics.

\paragraph{Why $\alpha$ differs by signal type.}
Within the NLP decoder family, $\alpha$ is stable across architectures; across modalities, $\alpha$ varies by signal type ($\alpha_\mathrm{NLP}\approx1.48$, $\alpha_\mathrm{ViT}\approx4.5$, $\alpha_\mathrm{CNN}\approx4.0$, $\alpha_\mathrm{audio}\approx4.7$). The audio-vision-ViT convergence ($\alpha\approx3.3$--$5.0$ for all continuous-signal modalities, vs.\ $\alpha\approx1.5$ for discrete-token NLP) suggests a two-regime structure.
Both regimes share the same equicorrelation ($\rho\approx0.46$, CV$=1.0\%$), so $\rho$ does not explain the difference.
The leading hypothesis is that continuous-signal representations (images, spectrograms) produce within-class covariance $\Sigma_W$ with higher effective rank than discrete-token representations, amplifying the Gumbel race slope through a modality-specific renormalization factor.
This is consistent with the synthetic validation: in isotropic noise ($\Sigma_W\propto I$), $\alpha$ is constant regardless of $\rho$; the cross-modality $\alpha$ variation must therefore originate in the \emph{structure} of $\Sigma_W$, not in the competition geometry $\rho$.
Bidirectionality adds a further factor: encoders with mean-pooled representations yield $\alpha_\mathrm{encoder}\approx7.7$, consistent with bidirectional attention creating denser representations than autoregressive decoders.
...
A cross-modal survey confirms $\rho\approx0.46$ in every modality tested: NLP decoders ($0.463$), audio speech models ($0.455$--$0.464$), vision models ($0.458$--$0.467$), and mouse cortex ($0.466$), with pooled CV$=1.0\%$ across all 6 modalities (\texttt{results/cti\_cross\_modal\_rho.json}).
This establishes near-simplex geometry as a \emph{universal} structural property of categorical representations, independent of substrate, training procedure, and signal type.
...
\item \textbf{Protein language models.} Seven protein LMs from 3 architecture families (ESM-2 at 4 sizes, ProtBERT, ProtBERT-BFD, ESM-1b; EC-number classification, $K=7$, 200 sequences/class) yield $\alpha=-1.17$, $r=-0.15$, $p=0.76$: the law \emph{fails} across protein architectures (\texttt{results/cti\_protein\_esm2.json}). Diagnosis: EC classes are extremely heterogeneous --- each spans thousands of structurally diverse enzymes --- so protein LMs achieve accuracy via within-class compactness and local neighborhood structure rather than global centroid separation. The law requires semantically coherent categories where nearest-centroid geometry drives classification.
\item \textbf{Human fMRI.} Eleven visual ROIs from the Natural Scenes Dataset (NSD, subject 01, 4489 images, $K=12$ COCO supercategories) yield chance-level balanced 1-NN accuracy ($q_\mathrm{1nn}\approx0.08$, chance $=0.083$); pooled across all ROIs and classes ($n=132$ points), $r=0.12$, $p=0.18$, non-significant (\texttt{results/cti\_nsd\_human\_fmri.json}). Raw BOLD voxel responses lack sufficient SNR for nearest-neighbor classification of semantic categories; successful fMRI decoding typically requires ridge regression and noise-ceiling correction. The law requires measurable above-chance 1-NN accuracy as a prerequisite. The law is validated on high-SNR neural measurements (spike trains, multi-electrode arrays) but not raw BOLD.
\end{enumerate}
...
Cross-modal tests confirm the same functional form in ViT ($R^2=0.964$), ResNet50 CNN ($r=0.749$), and 7 speech models from 4 architectures on Speech Commands ($r=0.898$, $\alpha_\mathrm{audio}=4.67$, $K=36$); the law extends to biological visual cortex (Allen Neuropixels: 30/32 sessions H1 PASS, all 32 showing positive $r$).
The Gumbel-race mechanism further extends to next-token prediction: $\kappa$ from the unembedding matrix alone predicts $\log\mathrm{CE}$ across 22 models and 4 architecture types ($r=-0.55$; $r=-0.70$ excluding one outlier), with vocabulary size dropping out ($\beta_\mathrm{gen}\approx0$) because softmax competition spontaneously reduces to $K_\mathrm{eff}\approx 2$--$3$.
...
Universality is of the \emph{functional form}: the constant $\alpha$ is signal-type-specific ($\alpha_\mathrm{NLP}\approx1.48$ for discrete tokens; $\alpha_\mathrm{ViT}\approx4.5$, $\alpha_\mathrm{CNN}\approx4.0$, $\alpha_\mathrm{audio}\approx4.7$ for continuous signals), with all continuous-signal modalities converging at $\alpha\approx3.3$--$5.0$ and discrete-token NLP at $\alpha\approx1.5$; cross-task drift and honest-scope limitations (including protein LM failure and human fMRI null) are enumerated in \S\ref{sec:limitations}.
Open directions include a renormalization theory for how constants transform across tasks and modalities.
The near-simplex equicorrelation $\rho\approx0.46$ is now confirmed across all six modalities tested: NLP decoders ($0.463$), audio speech models ($0.455$--$0.464$), vision ($0.458$--$0.467$), mouse cortex ($0.466$), and NLP encoders ($0.446$--$0.456$); pooled CV$=1.0\%$ (\texttt{cti\_cross\_modal\_rho.json}). This substrate-independent constant is consistent with the finding that diverse embedding models share a universal geometric structure \citep{jha2025geometry}.
This is the tightest invariant in the paper, tighter than $\alpha$'s within-decoder CV of $2.3\%$, and establishes near-simplex centroid geometry as a universal structural property of categorical representation regardless of substrate, signal type, or training.
This decouples two distinct universality questions: (1) near-simplex geometry is substrate-independent; (2) the cross-family shift in $\alpha$ ($\alpha_\mathrm{decoder}\approx1.48$ vs $\alpha_\mathrm{audio}\approx4.7$ vs $\alpha_\mathrm{encoder}\approx7$) is not explained by $\rho$. Recent work \citep{chen2025universal} independently shows that diverse networks converge on fewer than 10 universal latent dimensions aligned with the human brain, consistent with CTI's $d_\mathrm{eff,comp}\approx2$ regime. Identical mean pooling is used throughout, ruling out pooling protocol. ELECTRA-small (discriminative RTD pre-training, bidirectional attention) matches BERT-base (MLM, bidirectional) in $\alpha_\mathrm{emp}$ ($3.84$ vs $4.37$), ruling out pre-training objective; bidirectionality of the attention pattern is the leading candidate for the renormalization mechanism.
...
\bibitem[Zhang et~al.(2025)]{zhang2025gps}
Z.~Zhang, F.~Lin, H.~Liu, J.~Morales, H.~Zhang, K.~Yamada, V.~B.~Kolachalama, and V.~Saligrama.
\newblock {GPS}: A probabilistic distributional similarity with {G}umbel priors for set-to-set matching.
\newblock \emph{ICLR}, 2025.

\bibitem[Lourie et~al.(2025)]{lourie2025scaling}
N.~Lourie, M.~Y.~Hu, and K.~Cho.
\newblock Scaling laws are unreliable for downstream tasks: A reality check.
\newblock \emph{Findings of EMNLP}, 2025.

\bibitem[Jha et~al.(2025)]{jha2025geometry}
R.~Jha, C.~Zhang, V.~Shmatikov, and J.~X.~Morris.
\newblock Harnessing the universal geometry of embeddings.
\newblock \emph{arXiv:2505.12540}, 2025.

\bibitem[S\'{u}ken\'{i}k et~al.(2025)]{sukenik2025nc}
P.~S\'{u}ken\'{i}k, C.~H.~Lampert, and M.~Mondelli.
\newblock Neural collapse is globally optimal in deep regularized {ResNets} and {Transformers}.
\newblock \emph{arXiv:2505.15239}, 2025.
...
Table~\ref{tab:generation-law-models} lists all 22 models in the generation law analysis (Section~\ref{sec:generation-law}).
$\kappa_{\mathrm{top1K}}$ is computed from $W_U$ alone (zero forward passes); CE is measured on 20K tokens from WikiText-103 validation.
$K_{\mathrm{eff}}$ comes from the exact decomposition $\mathrm{CE}=\mathrm{margin}+\log K_{\mathrm{eff}}$.

\begin{table}[h]
\centering
\caption{Generation law: 22-model suite ordered by CE. Arch = architecture type.
$\kappa_{\mathrm{top1K}}$ and $\bar\kappa$ are from $W_U$ geometry; CE and $K_{\mathrm{eff}}$ from WikiText-103 inference.}
\label{tab:generation-law-models}
\small
\begin{tabular}{llrrrrrr}
\toprule
Model & Arch & $N$ (M) & $V$ & $\kappa_{\mathrm{top1K}}$ & $\bar\kappa$ & CE & $K_{\mathrm{eff}}$ \\
\midrule
Mistral-7B & Trans. & 7000 & 32K & 0.876 & 0.873 & 1.551 & 1.87 \\
Falcon-H1-3B & Hybrid & 3000 & 131K & 0.949 & 0.915 & 1.564 & 1.92 \\
Falcon-H1-1.5B & Hybrid & 1500 & 131K & 0.977 & 0.950 & 1.741 & 2.00 \\
Falcon-H1-0.5B & Hybrid & 500 & 131K & 0.716 & 0.820 & 1.935 & 2.17 \\
Mamba-2.8B & SSM & 2800 & 50K & 0.720 & 0.884 & 2.110 & 2.27 \\
Pythia-2.8B & Trans. & 2800 & 50K & 0.794 & 0.917 & 2.174 & 2.32 \\
SmolLM2-360M & Trans. & 360 & 49K & 0.725 & 0.761 & 2.179 & 2.32 \\
Granite-Micro & Hybrid & 500 & 49K & 0.773 & 0.934 & 2.183 & 1.87 \\
Mamba-1.4B & SSM & 1400 & 50K & 0.720 & 0.899 & 2.218 & 2.35 \\
Qwen2-0.5B & Trans. & 500 & 152K & 0.716 & 0.838 & 2.292 & 2.42 \\
Pythia-1.4B & Trans. & 1400 & 50K & 0.795 & 0.918 & 2.292 & 2.41 \\
Mamba-790M & SSM & 790 & 50K & 0.724 & 0.766 & 2.308 & 2.42 \\
Pythia-1B & Trans. & 1000 & 50K & 0.821 & 0.932 & 2.398 & 2.48 \\
Qwen3-4B & Trans. & 4000 & 152K & 0.710 & 0.898 & 2.422 & 1.55 \\
Mamba-370M & SSM & 370 & 50K & 0.711 & 0.759 & 2.455 & 2.52 \\
Pythia-410M & Trans. & 410 & 50K & 0.791 & 0.894 & 2.565 & 2.69 \\
Qwen3-1.7B & Trans. & 1700 & 152K & 0.698 & 0.911 & 2.588 & 1.63 \\
Qwen3-0.6B & Trans. & 600 & 152K & 0.673 & 0.861 & 2.777 & 2.13 \\
Mamba-130M & SSM & 130 & 50K & 0.697 & 0.670 & 2.808 & 2.95 \\
Pythia-160M & Trans. & 160 & 50K & 0.251 & 0.273 & 3.093 & 3.33 \\
GPT-2 & Trans. & 124 & 50K & 0.730 & 0.798 & 3.108 & 3.52 \\
LFM2.5-1.2B & Novel & 1200 & 66K & 0.863 & 0.947 & 3.349 & 2.42 \\
\bottomrule
\end{tabular}
\end{table}
