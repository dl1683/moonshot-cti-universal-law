...
Near-simplex competition geometry ($\rho\approx0.45$, CV$=3.9\%$ across NLP architectures, CV$=1.65\%$ across 5 cortical areas) provides a \emph{zero-parameter} prediction of $\alpha$ for NLP decoders: $\hat\alpha_\mathrm{pred}=\sqrt{4/\pi}\sqrt{d_\mathrm{eff,comp}}=1.525$ vs.\ observed $1.477$ (3.2\% error; the formula is derived from EVT, with only $\rho$ measured; encoder $\alpha$ is $4$--$5\times$ higher for the same $\rho$, indicating a second renormalization factor beyond $\rho$).
...
\item \textbf{Zero-parameter prediction (NLP decoders)}: The near-simplex equicorrelation ($\rho\approx0.45$, CV$=3.9\%$ across NLP decoders, CV$=1.65\%$ across 5 mouse V1 areas) enables an EVT-derived prediction of $\alpha$ with only $\rho$ as a measured structural input: $\hat\alpha_\mathrm{pred}=\sqrt{4/\pi}\sqrt{1/(1-\rho)}=1.525$ vs.\ observed $\alpha=1.477$ (3.2\% error). The same $\rho\approx0.45$ in biological cortex suggests near-simplex geometry is substrate-independent; however, for encoders with the same $\rho$, the formula underpredicts $\alpha$ by $4$--$5\times$, indicating a second renormalization factor (bidirectionality) beyond $\rho$.
\end{enumerate}
...
Measuring $\rho$ empirically on five architectures (Pythia-160m to OLMo-1B) and three datasets ($K=4,14,77$): $\rho\in[0.41,0.47]$, $d_\mathrm{eff,comp}\in[1.69,1.88]$, with CV$=3.9\%$ across architectures (tighter than $\alpha$'s CV$=2.3\%$).
