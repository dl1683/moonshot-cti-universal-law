...
Near-simplex competition geometry ($\rho\approx0.45$, CV$=3.9\%$ across NLP architectures, CV$=1.65\%$ across 5 cortical areas) provides a \emph{zero-parameter} prediction of $\alpha$: $\hat\alpha_\mathrm{pred}=\sqrt{4/\pi}\sqrt{d_\mathrm{eff,comp}}=1.525$ vs.\ observed $1.477$ (3.2\% error, zero free parameters).
...
\item \textbf{Zero-parameter prediction}: The near-simplex equicorrelation ($\rho\approx0.45$, CV$=3.9\%$ across NLP decoders, CV$=1.65\%$ across 5 mouse V1 areas) enables a zero-free-parameter prediction of $\alpha$: $\hat\alpha_\mathrm{pred}=\sqrt{4/\pi}\sqrt{1/(1-\rho)}=1.525$ vs.\ observed $\alpha=1.477$ (3.2\% error). The same $\rho$ value in biological cortex suggests near-simplex geometry is substrate-independent --- the critical link between the theory, the AI law, and the biological findings.
\end{enumerate}
...
\item \textbf{Beyond-1NN scope.} $\kappa_\mathrm{nearest}$ predicts $k=3,5$ NN accuracy ($r\approx0.79$) and silhouette ($r=0.88$) pooled; pre-registered primary criterion $r(\kappa_\mathrm{nearest}, q_\mathrm{linear})>0.7$ fails within each dataset ($r=0.22$--$0.59$). Linear-probe accuracy depends on global separability, distinct from nearest-neighbour directional SNR. However, within-model layer ranking passes a pre-registered downstream protocol (V3; \texttt{results/cti\_downstream\_protocol\_v3.json}): \textbf{H1\_new} ($\kappa_\mathrm{nearest}$ across layers vs.\ $q_{1\mathrm{NN}}$, $n=5$ models $\times 2$ datasets): mean Spearman $\rho=0.640$, 10/10 positive (PASS); \textbf{H2} (vs.\ MAP@10): mean $\rho=0.623$, 10/10 positive (PASS); \textbf{H3} (cross-model ranking on banking77, $K=77$, $n=5$): Spearman $\rho=0.700$ (PASS). These results confirm $\kappa_\mathrm{nearest}$ as a practical layer-selection \emph{and} architecture-ranking signal for retrieval tasks.
