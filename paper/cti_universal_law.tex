...
\textbf{(2) Constant universality within family}: $\alpha$ is stable within architecture families (NLP decoders: CV$=0.019$/$0.023$; 19 architectures); across families, $\alpha$ varies ($\alpha_\mathrm{NLP}\approx1.48$, $\alpha_\mathrm{ViT}\approx0.63$, $\alpha_\mathrm{CNN}\approx4.4$), with family identity the leading predictor.
...
Per-family values: $\alpha_\mathrm{decoder}\approx1.48$, $\alpha_\mathrm{encoder}\approx7.7$, $\alpha_\mathrm{ViT}\approx0.63$, $\alpha_\mathrm{CNN}\approx4.4$; the law applies to any architecture when appropriate pooling is used.
...
Universality is of the \emph{functional form}: the constant $\alpha$ is architecture-family-specific ($\alpha_\mathrm{NLP}\approx1.48$, $\alpha_\mathrm{ViT}\approx0.63$, $\alpha_\mathrm{CNN}\approx4.4$), with cross-task drift documented and honest-scope limitations enumerated in \S\ref{sec:limitations}.
