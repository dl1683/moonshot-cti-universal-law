...
Biological generalization: 30/32 mouse visual-cortex sessions satisfy the law ($\bar{r}=0.736$); near-simplex equicorrelation ($\rho\approx0.46$, CV$=1.0\%$ across 6 modalities: NLP, audio, vision, mouse cortex) provides a zero-parameter prediction of $\alpha$ ($+4.3\%$ error).
...
\textbf{Competition geometry.} The $\Sigma_W$-whitened equicorrelation $\rho$ across 5 cortical areas gives $\rho\in\{0.428, 0.439, 0.451, 0.462, 0.448\}$ (mean$=0.466$, CV$=1.65\%$), strikingly close to NLP decoders ($\rho_\mathrm{LM}=0.463$). A cross-modal survey confirms $\rho\approx0.46$ in every modality tested: NLP ($0.463$), audio ($0.455$--$0.464$), vision ($0.458$--$0.467$), and mouse cortex ($0.466$); pooled CV$=1.0\%$ across all 6 modalities --- the tightest invariant in this work. See Figure~\ref{fig:allen-bio}.
...
Synthetic validation with equicorrelated Gaussian data ($K\in\{4,10,20\}$, $d=50$, $\rho\in[0,0.8]$) confirms the logit-linear form (mean $R^2=0.982$), establishing it as a structural prediction, not a curve-fitting artifact. Importantly, $\alpha$ does not depend on $\rho$ under isotropic noise; the $\alpha(\rho)$ relationship in real networks requires structured $\Sigma_W$.
The two-regime alpha structure ($\alpha\approx1.5$ for discrete tokens vs.\ $\alpha\approx3.3$--$5.0$ for continuous signals) is not explained by $\rho$ (identical at $0.46$), but by modality-specific within-class covariance structure: continuous-signal representations produce higher effective-rank $\Sigma_W$, amplifying the Gumbel race slope.
...
A cross-modal survey confirms $\rho\approx0.46$ in every modality: NLP decoders ($0.463$), audio ($0.455$--$0.464$), vision ($0.458$--$0.467$), mouse cortex ($0.466$), and NLP encoders ($0.446$--$0.456$); pooled CV$=1.0\%$ across 6 modalities (supplementary materials). The cross-family $\alpha$ shift is not captured by $\rho$; bidirectionality is the leading candidate for the encoder renormalization.
