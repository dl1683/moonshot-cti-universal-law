...
Cross-modal validation confirms the same form for ViT-Large ($R^2=0.964$), ResNet50, and 7 frozen speech models on Speech Commands ($r=0.898$, $p=0.006$, $K=36$).
...
Biological generalization: 30/32 mouse visual-cortex sessions satisfy the law ($\bar{r}=0.736$).
The Gumbel-race mechanism extends to generation: $\kappa$ from the unembedding matrix $W_U$ alone predicts $\log\mathrm{CE}$ across 22 models ($r=-0.55$, $p=0.008$; $r=-0.70$ excluding one outlier).
...
Neural scaling laws established simple macroscopic regularities for loss as a function of compute and model size \citep{kaplan2020,hoffmann2022}, though unreliable for downstream tasks \citep{lourie2025scaling}. A parallel question for representation learning is whether there is an equally simple \emph{microscopic} law for class separability and nearest-neighbor classification quality across architectures.
...
\item \textbf{Cross-modal and causal validation}: ViT-Large $R^2=0.964$; ResNet50 CNN $r=0.749$; 7 speech models $r=0.898$ ($K=36$); pre-registered confusion-matrix causal prediction $r=0.842$--$0.776$ with zero refit; blind OOD $r=0.817$; expanded holdout (11 models $\times$ 8 datasets, $n=77$) all 6 criteria PASS ($r=0.879$, MAE$=0.077$).
\item \textbf{Biological validation}: 30/32 Allen Neuropixels mouse V1 sessions PASS; multi-area batch confirms $r>0.70$ across 5 cortical areas.
\item \textbf{Generation law}: $\kappa$ from $W_U$ (zero forward passes) predicts $\log\mathrm{CE}$ across 22 models ($r=-0.55$; $r=-0.70$ excluding one outlier; fixed-$V$: $r=-0.84$, architecture-independent). Vocabulary size drops out ($K_\mathrm{eff}\approx 2$--$3$).
\end{enumerate}
The law enables zero-cost diagnostics: $\kappa_\mathrm{nearest}$ from frozen embeddings ranks architectures by retrieval quality ($\rho=0.833$, $p=0.005$; pre-registered) and selects optimal representation layers without downstream labels. Four probe measurements reduce MAE by 86\%.
...
Classical EVT describes minima/maxima asymptotics \citep{fisher1928,gnedenko1943,gumbel1958}; \citet{zhang2025gps} independently confirm Gumbel priors for nearest-neighbor matching. Our derivation applies this to distance minima among competing classes. For generation, \citet{wu2024linguistic} find NC properties correlate with generalization in LLMs; \citet{kulkarni2026disentangling} show effective rank alone does not predict loss; our $\kappa$ outperforms by capturing competition-relevant geometry.
...
The former universal-equicorrelation, near-simplex-universality, and zero-parameter $\alpha(\rho)$ claims are retracted; Appendix~\ref{app:equicorr-retraction} records the historical measurement and the failed survival tests.
...
\toprule
Model & Family & Dataset & $K$ & $\alpha$ ($A$) & $R^2$/$r$ \\
\midrule
ViT-L-16-224 & ViT & CIFAR-10 & 10 & $A{=}0.630$$^\ddagger$ & $R^2{=}0.964$ \\
ViT-B-16-224 & ViT & CIFAR-100 & 100 & 3.899 & $r{=}0.773$ \\
...
ViT-Large at $K=10$ reaches $R^2=0.964$, matching the NLP fit in explained variance. The ViT CIFAR-10 entries report $A$ (not $\alpha$): they fit $\logit(q)=A\cdot\kappa\cdot\sqrt{d_\mathrm{eff}}+C$, factoring out the effective dimensionality explicitly.\footnote{$^\ddagger$The CIFAR-10 ViT entries report $A$ from $\logit(q)=A\cdot\kappa\cdot\sqrt{d_\mathrm{eff}}+C$, not $\alpha$ from $\logit(q)=\alpha\cdot\kappa+C$. When ViT is fitted with the standard parameterization, the combined cross-layer $\alpha{=}4.49$, consistent with CNN ($4.0$) and audio ($4.67$).} The \emph{functional form} (logit-linear in $\kappa$) is universal; the constant is signal-type-specific.

\paragraph{Audio modality.}
Seven frozen speech models from 4 architectures (Wav2Vec2, HuBERT, WavLM, Whisper; 39M--316M parameters) on Speech Commands v0.02 ($K=36$ spoken keywords, 4890 test samples; features at $\frac{2}{3}$-depth layer, PCA to 256 dimensions) yield $r=0.898$ ($p=0.006$), $\alpha_\mathrm{audio}=4.669$ ($R^2=0.807$; supplementary materials).
The slope $\alpha_\mathrm{audio}\approx4.7$ matches $\alpha_\mathrm{CNN}\approx4.4$ but not $\alpha_\mathrm{NLP}\approx1.5$, suggesting a two-regime structure: continuous-signal modalities (vision, audio) share $\alpha\approx4$--$5$, while discrete-token modalities (NLP) operate at $\alpha\approx1.5$.
...
11 unseen models $\times$ 8 datasets ($n=77$), all parameters frozen (pre-registered before data collection). All six criteria pass:
$r=0.879$ (H8a: $\geq0.85$),
...
\textbf{Retracted competition-geometry analysis.} The earlier equicorrelation and near-simplex interpretations of the Allen data used the same incorrect whitening transform and are retracted. The biological validation here concerns the association between $\kappa_\mathrm{nearest}$ and decoding performance; it does not support a universal equicorrelation parameter.
...
Left: All models ($n{=}22$, $r{=}{-}0.55$; regression line fit on $n{=}21$ excluding LFM2.5: $r{=}{-}0.697$).
Right: Fixed-$V$ (Pythia+Mamba, $n{=}10$; $r{=}{-}0.837$, architecture-independent).}
\label{fig:generation-law}
\end{figure}

\subsection{Scope of universality and architecture ranking}
\label{subsec:comprehensive-universality}

The law exhibits a three-level universality hierarchy:
\textbf{(1) Form}: logit-linear in $\kappa_\mathrm{nearest}$, confirmed across NLP, ViT, CNN, audio, and biological systems.
\textbf{(2) Constant within signal type}: $\alpha$ is stable within families (NLP decoders: CV$=0.023$; 19 architectures: CV$=1.75\%$); across families, $\alpha$ varies by signal type: discrete-token NLP decoders give $\alpha\approx1.48$, while all continuous-signal modalities converge at $\alpha\approx3.3$--$5.0$ (ViT: $4.5$, CNN: $4.0$, audio: $4.7$).
...
Seven strongly supported claims: (1) form universality (EVT-derived), (2) NLP constant stability (CV$_\alpha=0.019$--$1.75\%$ across 12--19 architectures), (3) architecture boundary crossing (RWKV pure RNN passes), (4) cross-family form universality (ViT and CNN yield comparable $r$ at $K=100$), (5) sparse competition exponent ($\beta=0.5$ beats $\beta=1.0$ by 10.1\% LOAO; unconstrained $\hat\beta=0.478$), (6) generation extension ($r=-0.55$ across 22 models, $r=-0.70$ excluding one outlier; architecture-independent within fixed $V$), (7) audio-visual alpha convergence: speech models from 4 architectures ($\alpha_\mathrm{audio}=4.67$, $r=0.898$, $p=0.006$) match vision CNNs ($\alpha_\mathrm{CNN}=4.4$), revealing a two-regime structure: continuous-signal modalities share $\alpha\approx4$--$5$, while discrete-token NLP operates at $\alpha\approx1.5$.
...
The Gumbel race is the same mechanism underlying EVT in physics.
Synthetic validation with equicorrelated Gaussian data ($K\in\{4,10,20\}$, $d=50$, generator parameter $\rho\in[0,0.8]$) supports the logit-linear form (mean $R^2=0.982$). The fitted $\alpha$ showed no systematic dependence on the generator's $\rho$, so this experiment provides no support for an $\alpha(\rho)$ prediction.
The two-regime alpha structure ($\alpha\approx1.5$ for discrete tokens vs.\ $\alpha\approx3.3$--$5.0$ for continuous signals) remains an empirical observation; its mechanism is open, and the retracted equicorrelation analysis cannot explain it.
...
\item \textbf{Protein language models.} Seven protein LMs from 3 architecture families (ESM-2 at 4 sizes, ProtBERT, ProtBERT-BFD, ESM-1b; EC-number classification, $K=7$) yield $\alpha=-1.17$, $r=-0.15$, $p=0.76$: the law \emph{fails} across protein architectures (supplementary materials). EC classes are extremely heterogeneous, so protein LMs achieve accuracy via within-class compactness rather than global centroid separation. The law requires semantically coherent categories.
\item \textbf{Human fMRI.} Eleven visual ROIs from the Natural Scenes Dataset (NSD, subject 01, $K=12$ COCO supercategories) yield chance-level 1-NN accuracy; pooled across all ROIs and classes ($n=132$), $r=0.12$, $p=0.18$, non-significant (supplementary materials). Raw BOLD voxel responses lack sufficient SNR for nearest-neighbor classification. The law is validated on high-SNR neural measurements (spike trains, multi-electrode arrays) but not raw BOLD.
\end{enumerate}
...
\section{Conclusion}
We present the CTI Universal Law, derived from extreme-value theory, with functional form proven as a conditional theorem. The law is validated across 12 NLP architectures (LOAO CV$_\alpha=0.023$), in blind OOD tests ($r=0.817$), an expanded holdout (11 models $\times$ 8 datasets, all 6 criteria pass), cross-modally (ViT $R^2=0.964$, ResNet50 $r=0.749$, 7 speech models $r=0.898$ with $\alpha_\mathrm{audio}=4.67$), and in biological visual cortex (30/32 sessions). The Gumbel-race mechanism extends to generation ($r=-0.55$, 22 models; $r=-0.70$ excluding one outlier). Universality is of the \emph{functional form}: the constant $\alpha$ is signal-type-specific ($\alpha_\mathrm{NLP}\approx1.48$ for discrete tokens; $\alpha_\mathrm{ViT}\approx4.5$, $\alpha_\mathrm{CNN}\approx4.0$, $\alpha_\mathrm{audio}\approx4.7$ for continuous signals), with all continuous-signal modalities converging at $\alpha\approx3.3$--$5.0$ and discrete-token NLP at $\alpha\approx1.5$; honest-scope limitations including protein LM failure and human fMRI null are enumerated in \S\ref{sec:limitations}. The formerly reported cross-modal equicorrelation constant, near-simplex universality, and $\alpha(\rho)$ explanation are retracted rather than retained as approximate claims. Open directions include a renormalization theory for cross-family constants and independent replication. Independent convergent evidence: \citet{styves2025} confirm centroid-separation geometry predicts decoding in human visual cortex.

\section*{Reproducibility Statement}
All numerical claims are taken directly from repository JSON artifacts. All pre-registrations were committed before data collection; timestamps are verifiable upon code release. Code and data will be released upon acceptance.
...
\paragraph{Retracted equicorrelation analysis.}
\label{app:equicorr-retraction}
For provenance, the legacy estimator reported six cross-modal values in $[0.455,0.467]$ with mean $0.462$ (CV$=1.0\%$; \texttt{results/cti\_cross\_modal\_rho.json}).
The code multiplied projected centroid differences by $\sqrt{\Lambda}$, applying $\Sigma_W^{1/2}$ rather than the stated inverse-whitening transform $\Sigma_W^{-1/2}$.
Matched labeled-cloud nulls place the historical band inside central 99\% null intervals, and a shared-anchor isotropic-centroid calculation yields $\rho\to1/2$ without learned or semantic structure.
No corrected real-data comparison is available because the retained caches lack aligned embeddings and labels.
Accordingly, the historical values are retained only as measurements from a faulty estimator; they do not support universal or approximately universal equicorrelation, near-simplex geometry, or a zero-parameter $\alpha(\rho)$ explanation.
...
\item \textbf{Protein language models.} Seven protein LMs from 3 families (ESM-2, ProtBERT, ESM-1b; EC classification $K=7$): $\alpha=-1.17$, $r=-0.15$, $p=0.76$ --- law fails. EC classes span thousands of structurally diverse enzymes; centroid separation is not the classification mechanism.
\item \textbf{Human fMRI.} NSD (11 visual ROIs, $K=12$ COCO supercategories): chance-level 1-NN ($q\approx0.08$); pooled $r=0.12$, $p=0.18$. Raw BOLD lacks SNR for nearest-neighbor classification; the law requires measurable above-chance 1-NN accuracy.
\end{enumerate}
