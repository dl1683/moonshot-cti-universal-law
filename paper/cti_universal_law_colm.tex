...
\item \textbf{Human fMRI.} Eleven visual ROIs from the Natural Scenes Dataset (NSD, subject 01, $K=12$ COCO supercategories) yield chance-level 1-NN accuracy; $r=0.33$, $p=0.32$, non-significant (\texttt{results/cti\_nsd\_human\_fmri.json}). Raw BOLD voxel responses lack sufficient SNR for nearest-neighbor classification. The law is validated on high-SNR neural measurements (spike trains, multi-electrode arrays) but not raw BOLD.
\end{enumerate}
...
\item \textbf{Human fMRI.} NSD (11 visual ROIs, $K=12$ COCO supercategories): chance-level 1-NN ($q\approx0.08$); $r=0.33$, $p=0.32$. Raw BOLD lacks SNR for nearest-neighbor classification; the law requires measurable above-chance 1-NN accuracy.
\end{enumerate}
