...
\item \textbf{Protein language models.} Seven protein LMs from 3 architecture families (ESM-2 at 4 sizes, ProtBERT, ProtBERT-BFD, ESM-1b; EC-number classification, $K=7$) yield $\alpha=-1.17$, $r=-0.15$, $p=0.76$: the law \emph{fails} across protein architectures (\texttt{results/cti\_protein\_esm2.json}). EC classes are extremely heterogeneous, so protein LMs achieve accuracy via within-class compactness rather than global centroid separation. The law requires semantically coherent categories.
\item \textbf{Human fMRI.} Eleven visual ROIs from the Natural Scenes Dataset (NSD, subject 01, $K=12$ COCO supercategories) yield chance-level 1-NN accuracy; pooled across all ROIs and classes ($n=132$), $r=0.12$, $p=0.18$, non-significant (\texttt{results/cti\_nsd\_human\_fmri.json}). Raw BOLD voxel responses lack sufficient SNR for nearest-neighbor classification. The law is validated on high-SNR neural measurements (spike trains, multi-electrode arrays) but not raw BOLD.
\end{enumerate}
...
A cross-modal survey confirms $\rho\approx0.46$ in every modality: NLP decoders ($0.463$), audio ($0.455$--$0.464$), vision ($0.458$--$0.467$), mouse cortex ($0.466$), and NLP encoders ($0.446$--$0.456$); pooled CV$=1.0\%$ across 6 modalities (\texttt{cti\_cross\_modal\_rho.json}). The cross-family $\alpha$ shift is not captured by $\rho$; bidirectionality is the leading candidate for the encoder renormalization.
