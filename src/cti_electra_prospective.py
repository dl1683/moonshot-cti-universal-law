...
"""
ELECTRA-SMALL PROSPECTIVE VALIDATION (Feb 21 2026)
===================================================
TRUE PROSPECTIVE TEST: ELECTRA-small (Google, 14M params) was NOT used to
fit any parameters. ELECTRA uses DISCRIMINATIVE pretraining (different from
both MLM=BERT and CLM=GPT-2/Phi-2):

  Generator: masked language model (like BERT, but smaller)
  Discriminator: detects which tokens were replaced by the generator

This is the ELECTRA discriminator model. It learns to distinguish real vs
fake tokens - a completely different objective than next-token prediction or
masked token prediction.

If the same alpha=1.54 holds for ELECTRA:
  - The law is independent of PRETRAINING OBJECTIVE (MLM, CLM, Discriminative)
  - Consistent with "representations converge to similar geometry under any task"
  - Strong evidence for universality claim

Model: google/electra-small-discriminator
  - 14M parameters (fastest model we've tested)
  - 12 layers, d=256 (different from BERT's d=768)
...
def main():
    print("=" * 70)
    print("ELECTRA-SMALL PROSPECTIVE VALIDATION (Discriminative pretraining)")
    print(f"Model: {MODEL_NAME} (14M params, UNSEEN during fitting)")
    ...
    for ds_name, ds_cfg in DATASETS.items():
        ...
        per_ds_cache = f"results/kappa_near_cache_{ds_name}_electra-small.json"
    ...
    # EVALUATION
    print("\n" + "=" * 70)
    print("ELECTRA-SMALL PROSPECTIVE VALIDATION RESULTS")
    print("=" * 70)
    ...
    output = {
        "experiment": "electra_small_prospective_validation",
        "architecture": "ELECTRA-small-discriminator (discriminative pretraining, NOT MLM/CLM)",
        "pre_registered": {
    ...
    out_path = "results/cti_electra_prospective.json"
