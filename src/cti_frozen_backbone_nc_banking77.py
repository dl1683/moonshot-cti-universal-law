...
"""
FROZEN-BACKBONE NC-LOSS CAUSAL TEST — BANKING77 K=77 NON-SATURATED REGIME
==========================================================================

Variant using Banking77 K=77 to achieve non-saturated q for quantitative H4 test.

MOTIVATION (from cti_frozen_backbone_nc_hard.py):
  DBpedia K=14 with proj_dim=13: q converges to 0.959±0.002 (near ceiling).
  NC effect on kappa confirmed (H3 PASS 5/5), but H4 (logit-ratio ~ alpha)
  is untestable because logit(q) is too sensitive to noise near ceiling.

NON-SATURATED DESIGN:
  Banking77 K=77 (intent classification, 77 fine-grained classes)
  proj_dim = K-1 = 76 (minimum for K=77 simplex)
  pythia-160m layer 0 (token embeddings — best layer for banking77 per prior)
  kappa_nearest=0.320 at best layer → expected q in 0.4-0.7 range
  N_TRAIN=1540 (20 per class), N_TEST=770 (10 per class) — keeps it manageable

QUANTITATIVE PREDICTION (H4):
  If NC moves kappa from k_CE to k_NC, then:
  delta_logit_q = alpha_CTI * (k_NC - k_CE) = 1.477 * delta_kappa
  Tolerance: |ratio - 1.477| / 1.477 < 0.50

FIXES from prior work:
  FIX 1: class means DIFFERENTIABLE (batch-scatter, no detach)
  FIX 2: q/kappa measured in proj_head space (same as optimization)
  FIX 3: KNN fit on TRAIN, score on TEST (not same set)
  FIX 4: Banking77 instead of CIFAR (no coarse-label confound)

PRE-REGISTERED HYPOTHESES:
  H1: mean delta_q(full_NC) > 0 AND >= 3/5 seeds positive
  H2: mean delta_q(shuffled_NC) <= mean delta_q(CE)
  H3: delta_kappa(NC) > 0 for >= 4/5 seeds
  H4: delta_logit_q / delta_kappa ~ 1.477 within 50%
...
BEST_LAYER = 0       # layer 0 = best for banking77 per prior K-sweep results
DATASET = "banking77"
K = 77
N_TRAIN = 1540       # 20 per class (manageable, low q baseline)
N_TEST = 770         # 10 per class
MAX_LENGTH = 64      # shorter sequences sufficient for banking77 intents

PROJ_DIM = K - 1     # = 76: minimum dim for K=77 simplex
N_EPOCHS = 100
BATCH_SIZE = 256
LR = 5e-4
N_SEEDS = 5
...
RESULT_PATH = RESULTS_DIR / "cti_frozen_backbone_nc_banking77.json"
...
def extract_embeddings(model_id, best_layer, dataset_name, n_train, n_test, max_length):
    ...
    def stratified_sample(ds, n_total, label_col="label", text_col="text"):
...
def main():
    print("FROZEN-BACKBONE NC-LOSS CAUSAL TEST — BANKING77 NON-SATURATED REGIME", flush=True)
    print(f"Dataset: {DATASET} K={K}, Arch: {ARCH} layer {BEST_LAYER}", flush=True)
    print(f"proj_dim={PROJ_DIM} (K-1={K-1}; natural non-saturated q via harder task)", flush=True)
