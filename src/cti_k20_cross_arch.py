#!/usr/bin/env python -u
"""
K=20 CROSS-ARCHITECTURE ABCS
================================

PRE-REGISTERED (Feb 23 2026).

Tests ABCS on K=20 (20newsgroups) with two architectures to cross-validate:
1. deberta-base  (768d, causal_v2 format)
2. qwen3-0.6b   (1024d, causal_v2 format)

Reference: DBpedia K=14 (5 archs) showed K_eff_estimated = 10 universally.
For K=20, if K_eff_estimated ≈ 10 => H_Kfix; if K_eff_estimated ≈ 14 => H_Kscale.

Also tests: does the cross-arch pattern from K=14 hold for K=20?
"""

import os
import json
import numpy as np
from sklearn.neighbors import KNeighborsClassifier
from scipy.stats import pearsonr
import scipy.special as sp
import datetime

EMBED_FILES = [
    ("deberta-base",  "results/causal_v2_embs_deberta-base_20newsgroups.npz"),
    ("qwen3-0.6b",    "results/causal_v2_embs_qwen3-0.6b_20newsgroups.npz"),
]

RESULT_PATH = "results/cti_k20_cross_arch.json"
LOG_PATH = "results/cti_k20_cross_arch_log.txt"

A_RENORM_K20 = 1.0535   # pre-registered for K=20
R_LEVELS = [0.3, 0.5, 1.0, 2.0, 5.0, 10.0]
M_BUNDLE = [1, 2, 3, 4, 5, 6, 7, 8, 10, 12, 15, 19]
N_TRAIN_PER_CLASS = 150  # 5000/20=250/class => 150 train, 100 test
RANDOM_SEED = 42

os.makedirs("results", exist_ok=True)
_log_fh = open(LOG_PATH, 'w')

def log(msg):
    print(msg, flush=True)
    _log_fh.write(msg + '\n')
    _log_fh.flush()


def load_and_split(path, n_train, seed):
    data = np.load(path)
    X, y = data['X'].astype(np.float64), data['y']
    classes = np.unique(y)
    rng = np.random.default_rng(seed)
    X_tr_list, y_tr_list, X_te_list, y_te_list = [], [], [], []
    for c in classes:
        idx = np.where(y == c)[0]
        rng.shuffle(idx)
        n = min(n_train, len(idx) - 1)
        X_tr_list.append(X[idx[:n]]); y_tr_list.append(y[idx[:n]])
        X_te_list.append(X[idx[n:]]); y_te_list.append(y[idx[n:]])
    return (np.concatenate(X_tr_list), np.concatenate(y_tr_list),
            np.concatenate(X_te_list), np.concatenate(y_te_list), classes)


def compute_geometry_full(X_tr, y_tr, classes):
    K = len(classes)
    N, d = len(X_tr), X_tr.shape[1]
    mu = np.stack([X_tr[y_tr == c].mean(0) for c in classes])
    grand_mean = X_tr.mean(0)

    trW = 0.0
    for i, c in enumerate(classes):
        Xc_c = X_tr[y_tr == c] - mu[i]
        trW += float(np.sum(Xc_c ** 2)) / N
    sigma_W_global = float(np.sqrt(trW / d))

    pair_info = []
    for i in range(K):
        for j in range(i + 1, K):
            Delta = mu[i] - mu[j]
            delta_ij = float(np.linalg.norm(Delta))
            Delta_hat = Delta / (delta_ij + 1e-10)
            kappa_ij = float(delta_ij / (sigma_W_global * np.sqrt(d) + 1e-10))

            sigma_cdir_sq = 0.0
            for k, c in enumerate(classes):
                Xc_c = X_tr[y_tr == c] - mu[k]
                n_c = len(Xc_c)
                proj = Xc_c @ Delta_hat
                sigma_cdir_sq += (n_c / N) * float(np.mean(proj ** 2))

            d_eff_ij = float(trW / (sigma_cdir_sq + 1e-10))
            kappa_eff_ij = kappa_ij * float(np.sqrt(d_eff_ij))

            pair_info.append({
                'i': int(i), 'j': int(j),
                'delta': float(delta_ij), 'kappa': float(kappa_ij),
                'd_eff': float(d_eff_ij), 'kappa_eff': float(kappa_eff_ij),
                'Delta_hat': Delta_hat,
            })

    pair_info.sort(key=lambda x: x['kappa'])
    nearest = pair_info[0]
    return {
        'mu': mu, 'grand_mean': grand_mean, 'trW': trW,
        'sigma_W_global': sigma_W_global, 'K': K, 'd': d,
        'pair_info': pair_info, 'nearest': nearest,
        'kappa': nearest['kappa'], 'd_eff': nearest['d_eff'],
        'kappa_eff': nearest['kappa_eff'], 'Delta_hat': nearest['Delta_hat'],
    }


def build_between_class_basis(mu, grand_mean, Delta_hat_first):
    K = mu.shape[0]
    basis = [Delta_hat_first.copy()]
    mu_c = mu - grand_mean
    for k in range(K):
        v = mu_c[k].copy()
        for b in basis:
            v -= (v @ b) * b
        norm_v = np.linalg.norm(v)
        if norm_v > 1e-8:
            basis.append(v / norm_v)
        if len(basis) == K - 1:
            break
    return np.stack(basis, axis=1)


def build_bundle_directions(pair_info, m):
    selected = pair_info[:m]
    d = selected[0]['Delta_hat'].shape[0]
    basis = []
    for pair in selected:
        v = pair['Delta_hat'].copy()
        for b in basis:
            v -= (v @ b) * b
        norm_v = np.linalg.norm(v)
        if norm_v > 1e-8:
            basis.append(v / norm_v)
    if not basis:
        return np.zeros((d, 1))
    return np.stack(basis, axis=1)


def compute_subspace_trace(X_tr, y_tr, mu, classes, U_directions):
    N = len(X_tr)
    tr = 0.0
    for k_vec in U_directions.T:
        sigma_sq = 0.0
        for i, c in enumerate(classes):
            Xc_c = X_tr[y_tr == c] - mu[i]
            n_c = len(Xc_c)
            proj = Xc_c @ k_vec
            sigma_sq += (n_c / N) * float(np.mean(proj ** 2))
        tr += sigma_sq
    return tr


def apply_bundle_surgery(X_tr, X_te, y_tr, y_te, geo, U_bundle, U_B, r, tr_W_null):
    mu = geo['mu']
    classes = np.unique(y_tr)
    tr_bundle = compute_subspace_trace(X_tr, y_tr, mu, classes, U_bundle)
    tr_null_new = tr_W_null + tr_bundle * (1.0 - 1.0 / r)
    scale_null = float(np.sqrt(tr_null_new / tr_W_null)) if tr_W_null > 1e-12 and tr_null_new > 0 else 1.0
    scale_bundle = 1.0 / float(np.sqrt(r))

    def transform(X, y):
        X_new = X.copy()
        for i, c in enumerate(classes):
            mask = (y == c)
            z = X[mask] - mu[i]
            z_between = z @ U_B @ U_B.T
            z_bundle = z @ U_bundle @ U_bundle.T
            z_other = z_between - z_bundle
            z_null = z - z_between
            X_new[mask] = mu[i] + scale_bundle * z_bundle + z_other + scale_null * z_null
        return X_new

    return transform(X_tr, y_tr), transform(X_te, y_te)


def eval_q(X_tr, y_tr, X_te, y_te, K):
    knn = KNeighborsClassifier(n_neighbors=1, metric='euclidean', n_jobs=1)
    knn.fit(X_tr, y_tr)
    acc = knn.score(X_te, y_te)
    q = float(np.clip((acc - 1.0/K) / (1.0 - 1.0/K), 1e-6, 1-1e-6))
    return acc, q, float(sp.logit(q))


def run_arch(arch_name, embed_path):
    log(f"\n{'='*70}")
    log(f"ARCH: {arch_name} / 20newsgroups K=20")
    log(f"{'='*70}")

    X_tr, y_tr, X_te, y_te, classes = load_and_split(embed_path, N_TRAIN_PER_CLASS, RANDOM_SEED)
    K = len(classes)
    d = X_tr.shape[1]
    log(f"K={K}, d={d}, n_train={len(X_tr)}, n_test={len(X_te)}")

    geo = compute_geometry_full(X_tr, y_tr, classes)
    acc_base, q_base, logit_base = eval_q(X_tr, y_tr, X_te, y_te, K)
    log(f"Baseline: acc={acc_base:.4f}, q={q_base:.4f}, logit={logit_base:.4f}")
    log(f"kappa={geo['kappa']:.4f}, d_eff={geo['d_eff']:.4f}")

    m_bundle = [m for m in M_BUNDLE if 1 <= m < K]

    U_B = build_between_class_basis(geo['mu'], geo['grand_mean'], geo['Delta_hat'])
    tr_bundle_all = compute_subspace_trace(X_tr, y_tr, geo['mu'], classes, U_B)
    tr_W_null = geo['trW'] - tr_bundle_all

    # Pre-registered constant
    A_keff = A_RENORM_K20 / float(np.sqrt(geo['d_eff']))
    C_const = logit_base - A_keff * geo['kappa_eff']

    slopes_by_m = {}
    for m in m_bundle:
        U_bundle = build_bundle_directions(geo['pair_info'], m)
        actual_m = U_bundle.shape[1]
        m_results = []
        for r in R_LEVELS:
            X_tr_b, X_te_b = apply_bundle_surgery(
                X_tr, X_te, y_tr, y_te, geo, U_bundle, U_B, r, tr_W_null)
            acc_r, q_r, logit_r = eval_q(X_tr_b, y_tr, X_te_b, y_te, K)
            delta_obs = logit_r - logit_base
            kappa_eff_pred = geo['kappa_eff'] * float(np.sqrt(r))
            delta_pred = (C_const + A_keff * kappa_eff_pred) - logit_base
            ratio = float(delta_obs / delta_pred) if abs(delta_pred) > 1e-6 else 0.0
            m_results.append({
                'r': float(r), 'actual_bundle_rank': int(actual_m),
                'delta_logit': float(delta_obs), 'delta_pred': float(delta_pred), 'ratio': ratio,
            })
            if abs(r - 1.0) > 0.01:
                log(f"  m={m:2d}, r={r:5.1f}: delta_obs={delta_obs:+.4f}, ratio={ratio:.3f}")

        nontrivial = [x for x in m_results if abs(x['r'] - 1.0) > 0.01]
        if len(nontrivial) >= 3:
            deltas_obs = [x['delta_logit'] for x in nontrivial]
            deltas_pred = [x['delta_pred'] for x in nontrivial]
            slope = float(np.polyfit(deltas_pred, deltas_obs, 1)[0]) if np.std(deltas_pred) > 1e-8 else 0.0
            r_val = float(pearsonr(deltas_obs, deltas_pred)[0]) if np.std(deltas_pred) > 1e-8 else 0.0
        else:
            slope, r_val = 0.0, 0.0
        slopes_by_m[m] = {'slope': slope, 'pearson_r': r_val}

    log(f"\n{'m':>4} | {'slope':>8} | {'pearson_r':>9}")
    for m, info in sorted(slopes_by_m.items()):
        log(f"{m:>4} | {info['slope']:>8.4f} | {info['pearson_r']:>9.4f}")

    m_vals = sorted(slopes_by_m.keys())
    slope_vals = [slopes_by_m[m]['slope'] for m in m_vals]

    n_for_pearson = min(8, len(m_vals))
    r_scale = float(pearsonr(m_vals[:n_for_pearson], slope_vals[:n_for_pearson])[0]) if n_for_pearson >= 3 else 0.0

    max_slope = max(slope_vals) if slope_vals else 0.0
    k_eff_est = None
    for m, s in zip(m_vals, slope_vals):
        if s >= 0.90 * max_slope:
            k_eff_est = m
            break

    slope_m1 = slopes_by_m.get(1, {}).get('slope', 1e-6)
    k_eff_from_single = 1.0 / (slope_m1 + 1e-10)

    log(f"\nr_scale = {r_scale:.4f} | K_eff_est = {k_eff_est} | K_eff_single = {k_eff_from_single:.2f}")

    return {
        'arch': arch_name,
        'd': int(d),
        'K': int(K),
        'baseline': {'acc': float(acc_base), 'q': float(q_base), 'logit': float(logit_base),
                     'kappa': float(geo['kappa']), 'd_eff': float(geo['d_eff']),
                     'kappa_eff': float(geo['kappa_eff'])},
        'slopes_by_m': {str(k): v for k, v in slopes_by_m.items()},
        'analysis': {
            'r_scale': float(r_scale),
            'k_eff_estimated': int(k_eff_est) if k_eff_est else None,
            'k_eff_from_single': float(k_eff_from_single),
            'pass_A1': bool(r_scale > 0.90),
        },
    }


def main():
    log("=" * 70)
    log("K=20 CROSS-ARCHITECTURE ABCS (20newsgroups)")
    log(f"Timestamp: {datetime.datetime.now().isoformat()}")
    log("=" * 70)

    results = []
    for arch_name, embed_path in EMBED_FILES:
        if not os.path.exists(embed_path):
            log(f"MISSING: {embed_path}")
            continue
        result = run_arch(arch_name, embed_path)
        results.append(result)

    log("\n" + "=" * 60)
    log("SUMMARY: K=20 Cross-Architecture")
    log("=" * 60)
    log(f"\n{'Arch':>15} | {'d':>6} | {'K_eff_est':>9} | {'K_eff_single':>12} | A1")
    for r in results:
        a = r['analysis']
        log(f"{r['arch']:>15} | {r['d']:>6} | {str(a['k_eff_estimated']):>9} | {a['k_eff_from_single']:>12.2f} | {'YES' if a['pass_A1'] else 'NO'}")

    k_eff_vals = [r['analysis']['k_eff_estimated'] for r in results if r['analysis']['k_eff_estimated']]
    if k_eff_vals:
        log(f"\nK=20: K_eff_estimated = {k_eff_vals}")
        log(f"Compare to DBpedia K=14: K_eff ~ 10")
        if all(k >= 12 for k in k_eff_vals):
            conclusion = "K_eff scales with K (K=20 -> K_eff>=12). H_KSCALE supported."
        elif all(k <= 11 for k in k_eff_vals):
            conclusion = "K_eff fixed (~10) for K=20. H_KFIX supported."
        else:
            conclusion = f"Mixed: K_eff range {min(k_eff_vals)}-{max(k_eff_vals)} for K=20."
        log(f"CONCLUSION: {conclusion}")

    output = {
        'experiment': 'k20_cross_arch_abcs',
        'timestamp': datetime.datetime.now().isoformat(),
        'results': results,
        'verdict': conclusion if k_eff_vals else 'INSUFFICIENT DATA',
    }
    with open(RESULT_PATH, 'w') as f:
        json.dump(output, f, indent=2)
    log(f"\nSaved to {RESULT_PATH}")


if __name__ == "__main__":
    main()
    _log_fh.close()
