...
"""
K=7 MULTI-KAPPA SWEEP: B(kappa) = C / kappa^p
==============================================
PURPOSE:
  Directly test the composite law B(kappa) = C/kappa^p with
  K=7 classes (avoids K=14 easy-class ceiling problem).

  Prior evidence (K=14 synthetic):
    v1 (kappa=0.40): B=6.11, CV=5.3%  (4 valid alpha levels)
    v2 (kappa=0.20): B=10.16, CV=3.9% (3 valid alpha levels)
    v3 (kappa=0.30): B=7.25, CV=56%   (6 valid, noisy - ceiling problem)
  Fit: C=3.12, p=0.733 from v1+v2 only. C is close to pi=3.14159.
  v3 PR1 PASS: B_obs=7.25 vs B_pred=7.55 (3.8% error) -- kappa-scaling confirmed.
  But within-kappa B is noisy at kappa=0.30 due to K=14 ceiling.

  FIX: K=7 has fewer easy classes -> lower q_base -> cleaner surgery signal.

DESIGN:
  - K=7, d=768, N_PER_CLASS=200
...
PRE-REGISTERED (Feb 23, 2026 -- committed BEFORE running):
  Calibration: kappa=[0.15, 0.20] (use these to fit C and p)
  Test: kappa=[0.25, 0.30, 0.40, 0.50] (held-out, predict from calibration)

  From v1+v2 calibration (K=14):
    C=3.12, p=0.733
    B_pred(0.15) = 3.12/0.15^0.733 = 3.12/0.207 = 15.07
    B_pred(0.20) = 3.12/0.20^0.733 = 3.12/0.307 = 10.16
    B_pred(0.25) = 3.12/0.25^0.733 = 3.12/0.392 = 7.96
    B_pred(0.30) = 3.12/0.30^0.733 = 3.12/0.413 = 7.55
    B_pred(0.40) = 3.12/0.40^0.733 = 3.12/0.511 = 6.11
    B_pred(0.50) = 3.12/0.50^0.733 = 3.12/0.603 = 5.18

  PR1: For each kappa, |B_obs - B_pred| / B_pred < 0.30 (within 30%)
       [relaxed due to K=7 potentially having different C]
  PR2: log-log slope of B vs kappa in [-0.90, -0.55] (p in [0.55, 0.90])
  PR3: fit C_obs and p_obs from 6 kappa points, CV of C-normalized B values < 0.15
  ...
  PRIMARY CRITERION: PR5 (directional test) + PR2 (slope) must both pass.
  SECONDARY: PR4 (within-kappa consistency) + PR3 (tight law)
"""
...
RESULT_PATH = "results/cti_k7_kappa_sweep.json"
LOG_PATH = "results/cti_k7_kappa_sweep_log.txt"
...
# Prior predictions (from K=14 v1+v2 calibration: C=3.12, p=0.733)
C_PRIOR = 3.12
P_PRIOR = 0.733
C_PI = np.pi
...
def generate_synthetic_embeddings(K, d, n_per_class, Sigma, kappa_nearest, seed):
    ...
    # K=7: 5 extra classes at linspace(1.5, 4.0) * delta_min
...
def b_pred_from_prior(kappa):
    """Predict B = A_local * sqrt(d_eff_1) for given kappa using K=14 prior."""
    return C_PRIOR / (kappa ** P_PRIOR)


def main():
    log("=" * 70)
    log("K=7 MULTI-KAPPA SWEEP: B(kappa) = C / kappa^p")
    ...
    log(f"  Prior from K=14 v1+v2: C={C_PRIOR}, p={P_PRIOR}")
    for k in KAPPA_VALUES:
        log(f"  B_pred(kappa={k}) = {b_pred_from_prior(k):.3f}")
    ...
    # Store results: kappa -> alpha -> {mean_B, std_B, mean_r_wk, ...}
    all_kappa_results = {}
    ...
    for kappa in KAPPA_VALUES:
        ...
        log(f"  B_pred(prior) = {B_pred:.3f}, B_pred_pi = {C_PI/kappa**P_PRIOR:.3f}")

        kappa_results = {}  # alpha -> seed_results
        ...
        # Aggregate across alpha levels for this kappa
        ...
        log(f"\n  KAPPA={kappa} AGGREGATE: B={B_kappa_mean:.4f}+/-{B_kappa_std:.4f}"
            ...
            f"rel_err={abs(B_kappa_mean-B_pred)/B_pred:.3f} (valid_alpha={n_valid_alpha}/3)")
    ...
    if len(kappas_with_data) < 3:
        ...
        log(f"\nPR2 slope = {slope:.4f}, C_fit = {C_fit:.4f} -> "
            f"{'PASS' if pr2 else 'FAIL'} (slope in [-0.90,-0.55])")
        log(f"   C_fit vs C_prior={C_PRIOR}: rel_err={abs(C_fit-C_PRIOR)/C_PRIOR:.3f}")
        log(f"   C_fit vs pi={C_PI:.4f}: rel_err={abs(C_fit-C_PI)/C_PI:.3f}")
        analysis['PR2'] = bool(pr2)
        analysis['slope'] = float(slope)
        analysis['C_fit'] = float(C_fit)
        analysis['p_fit'] = float(-slope)

        # PR3: CV of C-normalized B values < 0.15
        C_normalized = [all_kappa_results[k]['B_kappa_mean'] * (k ** (-slope))
                        for k in kappas_with_data]
        ...
        pr4 = n_clean_kappa >= 4
        log(f"\nPR4 kappa values with within-kappa CV < 0.20: {n_clean_kappa}/{len(kappas_with_data)}"
            f" -> {'PASS' if pr4 else 'FAIL'} (>= 4/6)")
        ...
        pr1 = n_pr1 >= len(kappas_with_data) * 0.67  # 2/3 of kappas pass individually
        log(f"\nPR1 kappa values within 30% of prior: {n_pr1}/{len(kappas_with_data)}"
            f" -> {'PASS' if pr1 else 'FAIL'} (need >= 2/3)")
        analysis['PR1'] = bool(pr1)
        analysis['n_pr1_pass'] = int(n_pr1)

        # Verdict: PRIMARY = PR5 + PR2 both pass
        primary_pass = pr5 and pr2
        n_secondary = sum([pr3, pr4, pr1])
        if primary_pass and n_secondary >= 2:
            verdict = "STRONG_PASS"
        elif primary_pass:
            verdict = "PASS"
        elif pr5 or pr2:
        ...
        log(f"SECONDARY (PR1,PR3,PR4): {n_secondary}/3 pass")
        log(f"VERDICT: {verdict}")
        log(f"\nFitted law: B(kappa) = {C_fit:.3f} / kappa^{-slope:.3f}")
        log(f"Prior law:  B(kappa) = {C_PRIOR:.3f} / kappa^{P_PRIOR:.3f}")
        log(f"C_fit / pi = {C_fit/C_PI:.4f} (1.0 = exact pi)")
